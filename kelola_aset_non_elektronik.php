...
// Fungsi untuk menampilkan data Aset Non Elektronik
function show_non_elektronik($conn) {
    $sql = "SELECT * FROM aset_non_elektronik";
    ...
    // Fungsi untuk menambah data Aset Non Elektronik
...
<link rel="icon" type="image/png" sizes="16x16" href="images/labkom_logo.png" />
  <title>Kelola Aset Non Elektronik</title>
  <!-- bootstrap.min css -->
...
<body>
    <h1>Kelola Aset Non Elektronik</h1>
    <?php show_non_elektronik($conn); ?>
<!-- Modal untuk menampilkan form edit aset non elektronik -->
<div class="modal fade" id="editNonElektronikModal" tabindex="-1" role="dialog" aria-labelledby="editNonElektronikModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editNonElektronikModalLabel">Edit Aset</h5>
    ...
    <!-- Tombol "Tambah Aset" -->
    <button class="btn btn-primary custom-btn" id="tambah-nonelektronik-btn" style="width: 200px; height: 40px; font-size: 12px;">Tambah Aset</button>
    <!-- Modal for adding a new user -->
    <div class="modal fade" id="addNonElektronikModal" tabindex="-1" role="dialog" aria-labelledby="addNonElektronikModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addNonElektronikModalLabel">Tambah Aset</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                ...
                <form id="form-add-nonelektronik">
                    <div class="form-group">
...
<script>
    $(document).ready(function() {
        ...
        $(".edit-btn").click(function() {
            ...
            // Buka modal
            $("#editNonElektronikModal").modal("show");
        });
        ...
        $("#form-edit-pengguna").submit(function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "proses_edit_non_elektronik.php",
                data: formData,
                success: function(response) {
                    alert(response); // Tampilkan pesan sukses atau error dari server
                    $("#editNonElektronikModal").modal("hide"); // Tutup modal setelah sukses
        ...
        $("#tambah-nonelektronik-btn").click(function() {
            // Buka modal
            $("#addNonElektronikModal").modal("show");
        });

        // Handle aksi submit form tambah elektronik menggunakan Ajax
        $("#form-add-nonelektronik").submit(function(e) {
            e.preventDefault();
            var formData = $(this).serialize();
            $.ajax({
                type: "POST",
                url: "proses_tambah_non_elektronik.php",
                data: formData,
                success: function(response) {
                    alert(response); // Tampilkan pesan sukses atau error dari server
                    $("#addNonElektronikModal").modal("hide"); // Tutup modal setelah sukses
        ...
        if (confirmation) {
            $.ajax({
                type: "GET",
                url: "proses_hapus_non_elektronik.php",
